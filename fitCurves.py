...
#    print(bezCurve)
    ...
    if maxError < error**2:
        for i in range(20):
            ...
            maxErrorLine, splitPointLine = computeMaxError([points[0],(points+points[-1])/2, points[-1]], bezCurve, uPrime)
            if maxErrorLine < error:
    ...
    # BUG FIX
    ################
    if np.array_equal(points[splitPoint-1], points[splitPoint+1]):
	points[splitPoint+1] += [0.001, 0.001]
